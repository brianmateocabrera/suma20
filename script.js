// Clase que representa una carta con un valor y color
class Carta {
...
class Mazo {
    constructor(colores, valores) {
        ...
        this.cartas = [];
        this.crear();   // Crea todas las cartas posibles
        this.mezclar(); // Las mezcla al inicio
    }
    ...
    mezclar() {
        // Mezcla estilo Fisher-Yates (aleatorio y justo)
    ...
    sacarCarta() {
        return this.cartas.pop(); // Saca la última carta del mazo
    }
}
...
class Juego {
    ...
    siguienteTurno() {
        ...
        // Animación de movimiento visual
        setTimeout(() => {
    ...
    mostrarOpciones() {
        ...
        if (opciones.length === 0) {
            ...
            opciones.forEach((combo, index) => {
                ...
                btn.textContent = `Opción ${index + 1}: ${Juego.formatearCombinacion(combo, this.mesa)}`;
    ...
    // Combina los valores en un string como "4 + 8 + 8"
    static formatearCombinacion(combo, cartas) {
        return combo.map(i => cartas[i].valor).join(" + ");
    }

    // Encuentra combinaciones de cartas que sumen un valor objetivo (por defecto, 20)
    ...
    // Renderiza visualmente las cartas en las diferentes zonas (mazo, mesa, descarte)
    ...
    // Crea visualmente una carta (div) con sus colores y valores
    static renderCard(card, colorMap, index = null, seleccionadas = [], onSelect) {
        ...
        if (index !== null) {
            ...
            if (onSelect) div.onclick = () => onSelect(index);
        }

        return div;
    }

    // Crea animación de movimiento entre dos zonas
